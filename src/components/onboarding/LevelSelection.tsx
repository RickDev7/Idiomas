...
export function GermanLevelCard({
  ...
  return (
    <button
      ...
      className="w-full text-left rounded-[18px] px-3 py-2.5 min-h-[52px] transition-all duration-200 flex items-center gap-3"
      style={
        selected
          ? {
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-[14px] font-bold text-white leading-tight">{option.label}</span>
        <span className="block text-[12px] text-[#94A3B8] leading-snug mt-0.5">{option.desc}</span>
      </span>
...
export function LevelSelection({
  ...
  return (
    <div>
      ...
      <p className="mt-3 flex items-start justify-center gap-1.5 text-[12px] text-[#64748B] text-center leading-snug px-2">
        <span className="text-[#00F2FE] mt-0.5 shrink-0" aria-hidden>
          <IconShield size={14} />
        </span>
...
export function DiagnosticIntro({
  ...
  if (kind === 'recommended') {
    return (
      <div
        className="mt-3 rounded-[18px] p-3.5 animate-fade-in"
        style={{
          background: 'rgba(139,92,246,0.14)',
          border: '1px solid rgba(139,92,246,0.4)',
        }}
      >
        <h3 className="text-[15px] font-bold text-white">Vamos descobrir juntos.</h3>
        <p className="text-sm text-[#94A3B8] mt-1 leading-snug flex items-center gap-1.5">
  ...
  if (kind === 'suggested') {
    return (
      ...
      >
        <h3 className="text-[15px] font-bold text-white">Quer confirmar seu nível?</h3>
  ...
  return (
    <div className="mt-3 text-center animate-fade-in">
      <p className="text-[12px] text-[#64748B]">
        Opcional: um teste rápido de ~2 min para confirmar.
      </p>
      {onRequestTest && (
        <button
          type="button"
          ...
          className="mt-1 text-sm font-semibold text-[#00F2FE] min-h-11"
        >
          Fazer teste rápido
        </button>
      )}
    ...
    <div className="animate-fade-in">
      ...
      <div className="rounded-[22px] p-5" style={glassStyle}>
        <p className="text-sm text-[#CBD5E1] leading-relaxed">
          O professor avalia fala, compreensão e vocabulário com poucas perguntas — só para começar
          no lugar certo.
        </p>
      </div>
      <div className="mt-6 space-y-3">
        <button
          ...
          className="w-full min-h-14 rounded-full text-white font-semibold inline-flex items-center justify-center gap-2"
          style={{
        ...
        {!required && (
          <button
            ...
            className="w-full min-h-11 text-sm font-semibold text-[#94A3B8]"
          >
            Continuar assim
          </button>
        )}
...
export function DiagnosticResultView({
  ...
  const rows = [
    { label: 'Fala', value: skills.speaking },
    { label: 'Compreensão', value: skills.listening },
    { label: 'Vocabulário', value: skills.vocabulary },
    { label: 'Comunicação', value: skills.communication ?? skills.reading },
  ];
  return (
    <div className="animate-fade-in pt-2">
      <p className="dt-label">Nível estimado</p>
      <h1 className="text-[32px] font-bold tracking-tight text-white mt-1 font-[family-name:var(--font-display)]">
        {estimatedLabel}
      </h1>
      <p className="text-sm text-[#94A3B8] mt-1 leading-snug">
      ...
      <ul className="mt-5 space-y-2">
        {rows.map((r) => (
          <li
            key={r.label}
            className="flex items-center gap-3 rounded-[16px] px-3.5 py-2.5 min-h-[44px]"
            style={glassStyle}
          >
            <span className="flex-1 text-sm font-medium text-white">{r.label}</span>
            <span className="text-sm font-bold text-[#00F2FE]">{r.value}</span>
          </li>
        ))}
      </ul>
      <div className="mt-5 rounded-[18px] p-4" style={glassStyle}>
        <p className="text-[12px] text-[#64748B] font-medium">Seu próximo foco</p>
        <p className="text-[15px] font-semibold text-white mt-1">{nextFocus}</p>
      </div>
    </div>
  );
}
